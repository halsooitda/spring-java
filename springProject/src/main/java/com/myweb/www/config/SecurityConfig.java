...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		.antMatchers("/member/list").hasRole("ADMIN") //멤버리스트는 admin만
		.antMatchers("/", "/board/list", "/board/detail", "/resources/**", //js
				"/upload/**", "/comment/**", "/member/register", "/member/login").permitAll() //비회원일 때 허용되는 경로, permitAll=모두에게
		.anyRequest().authenticated(); //나머지는 로그인한 사용자만 허용 처리 (글쓰기, 수정하기 등)
		
		//커스텀 로그인 페이지를 구성
		http.formLogin()
		.usernameParameter("email") //username = email
		.passwordParameter("pwd") //password = pwd 설정
